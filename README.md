...
- **MCP server**: `ltm mcp` — stdio-based Model Context Protocol server that exposes the verbs above as tools to Claude Code, Cursor, Zed, Claude Desktop, Continue, or any MCP-aware client. Reuses the CLI's auth, config, schema validation, and redaction pre-flight.
